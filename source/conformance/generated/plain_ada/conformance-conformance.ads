...
package Conformance.Conformance is
   ...
   type Test_Status_Vector is tagged private
     ...
     Constant_Indexing => Get_Test_Status_Constant_Reference;
   ...
   type Test_Status is
     record
        ...
        Failure_Message : Ada.Strings.Unbounded.Unbounded_String;
        Matched_Name    : Ada.Strings.Unbounded.Unbounded_String;
     end record;

   type Optional_Test_Status  (Is_Set : Boolean := False) is
     record
        case Is_Set is
           when True =>
              Value : Conformance.Test_Status;
           when False =>
              null;
        end case;
     end record;

   function Length (Self : Test_Status_Vector) return Natural;

   procedure Clear (Self : in out Test_Status_Vector);

   procedure Append (Self : in out Test_Status_Vector; V    : Test_Status);

   type Test_Status_Variable_Reference
     (Element : not null access Test_Status) is null record
     with Implicit_Dereference => Element;

   not overriding function Get_Test_Status_Variable_Reference
    (Self  : aliased in out Test_Status_Vector;
     Index : Positive)
      return Test_Status_Variable_Reference
     with Inline;

   type Test_Status_Constant_Reference
     (Element : not null access constant Test_Status) is null record
     with Implicit_Dereference => Element;

   not overriding function Get_Test_Status_Constant_Reference
    (Self  : aliased Test_Status_Vector;
     Index : Positive)
      return Test_Status_Constant_Reference
     with Inline;

   type Failure_Set is
     record
        Test : Conformance.Test_Status_Vector;
     end record;
     ...
     record
        case Result is
           ...
           when Parse_Error_Kind =>
              Parse_Error : Ada.Strings.Unbounded.Unbounded_String;
           when Serialize_Error_Kind =>
              Serialize_Error : Ada.Strings.Unbounded.Unbounded_String;
           when Timeout_Error_Kind =>
              Timeout_Error : Ada.Strings.Unbounded.Unbounded_String;
           when Runtime_Error_Kind =>
              Runtime_Error : Ada.Strings.Unbounded.Unbounded_String;
           when Protobuf_Payload_Kind =>
              Protobuf_Payload : PB_Support.Basics.Stream_Element_Vector;
           when Json_Payload_Kind =>
              Json_Payload : Ada.Strings.Unbounded.Unbounded_String;
           when Skipped_Kind =>
              Skipped : Ada.Strings.Unbounded.Unbounded_String;
           when Jspb_Payload_Kind =>
              Jspb_Payload : Ada.Strings.Unbounded.Unbounded_String;
           when Text_Payload_Kind =>
              Text_Payload : Ada.Strings.Unbounded.Unbounded_String;
        end case;
     end record;
...
private
   ...
   for Test_Status'Read use Read_Test_Status;

   for Test_Status'Write use Write_Test_Status;

   type Test_Status_Array is array (Positive range <>) of aliased Test_Status;

   type Test_Status_Array_Access is access Test_Status_Array;

   type Test_Status_Vector is
     new Ada.Finalization.Controlled
     with record
        Data   : Test_Status_Array_Access;
        Length : Natural := 0;
     end record;

   overriding procedure Adjust (Self : in out Test_Status_Vector);

   overriding procedure Finalize (Self : in out Test_Status_Vector);
